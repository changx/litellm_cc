...
class ProviderRouter:
    ...
    def get_provider_for_model(model_name: str) -> str:
        ...
        if any(
            ...
            for prefix in ["gpt-", "text-", "davinci", "curie", "babbage", "ada"]
        ):
            return "openai"
    ...
    def get_adapter(model_name: str) -> Optional[BaseLLMAdapter]:
        ...
        if provider == "openai":
            ...
            return OpenAIAdapter(
                api_key=settings.openai_api_key, api_base=settings.openai_api_base
            )
            ...
            return AnthropicAdapter(
                api_key=settings.anthropic_api_key, api_base=settings.anthropic_api_base
            )
            ...
            return GoogleAdapter(
                api_key=settings.google_api_key, api_base=settings.google_api_base
            )
            ...
            return CohereAdapter(
                api_key=settings.cohere_api_key, api_base=settings.cohere_api_base
            )
            ...
            return OpenAIAdapter(  # Azure uses OpenAI-compatible format
                api_key=settings.azure_api_key, api_base=settings.azure_api_base
            )
            ...
            return OpenAIAdapter(  # Assume custom provider uses OpenAI format
                api_key=settings.custom_api_key, api_base=settings.custom_api_base
            )
    ...
    async def chat_completion(
        request: LLMRequest, endpoint_type: str = "chat"
    ) -> LLMResponse:
