...
class BillingManager:
    ...
    async def process_usage_and_bill(
        ...
        try:
            ...
            cost_breakdown = await self.cost_calculator.calculate_cost(
                model_name=model_name, usage_data=usage_data
            )
            ...
            if total_cost > 0:
                success = await self.account_repo.atomic_spend(
                    account.user_id, total_cost
                )
                if not success:
                    logger.error(
    ...
    async def log_failed_request(
        ...
        try:
            ...
            logger.info(
                f"Error request logged for user {account.user_id}: {error_message}"
            )
    ...
    async def check_budget_before_request(
        self, account: Account, model_name: str, request_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            estimated_cost = await self.cost_calculator.estimate_request_cost(
                model_name=model_name, request_data=request_data
            )
