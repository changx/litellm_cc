...
async def create_account(
    account_data: AccountCreate, _: str = Depends(authenticate_admin)
):
    try:
...
async def get_account(user_id: str, _: str = Depends(authenticate_admin)):
...
async def update_account(
    user_id: str, update_data: AccountUpdate, _: str = Depends(authenticate_admin)
):
...
async def create_api_key(
    apikey_data: ApiKeyCreate, _: str = Depends(authenticate_admin)
):
    try:
...
async def get_api_key(api_key: str, _: str = Depends(authenticate_admin)):
...
async def get_user_api_keys(user_id: str, _: str = Depends(authenticate_admin)):
...
async def update_api_key(
    api_key: str, update_data: ApiKeyUpdate, _: str = Depends(authenticate_admin)
):
...
async def create_or_update_model_cost(
    cost_data: ModelCostCreate, _: str = Depends(authenticate_admin)
):
    try:
        model_cost = await ModelCostRepository.create_or_update(cost_data)

        # Invalidate cache
        await cache_manager.publish_cache_invalidation(
            "modelcost", cost_data.model_name
        )
...
async def get_all_model_costs(_: str = Depends(authenticate_admin)):
    costs = await ModelCostRepository.get_all()
    return costs


@router.get("/costs/{model_name}", response_model=ModelCost)
async def get_model_cost(model_name: str, _: str = Depends(authenticate_admin)):
    cost = await ModelCostRepository.get_by_model_name(model_name)
    if not cost:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Model cost not found"
        )
    return cost
...
async def get_user_usage_logs(
    user_id: str, limit: int = 100, skip: int = 0, _: str = Depends(authenticate_admin)
):
...
async def get_api_key_usage_logs(
    api_key: str, limit: int = 100, skip: int = 0, _: str = Depends(authenticate_admin)
):
