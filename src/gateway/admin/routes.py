...
from gateway.database.repositories import AccountRepository, ApiKeyRepository, ModelCostRepository, UsageLogRepository
...
async def create_bulk_api_keys(
    request: BulkApiKeyCreateRequest,
    _: bool = Depends(get_admin_auth)
):
    ...
    try:
        ...
        for i in range(request.count):
            ...
            created_key = await apikey_repo.create_api_key(api_key)
            created_keys.append(ApiKeyResponse(
                api_key=created_key.api_key,
                user_id=created_key.user_id,
                key_name=created_key.key_name,
                is_active=created_key.is_active,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create bulk API keys: {str(e)}")
...
async def create_or_update_model_cost(
    request: ModelCostCreateRequest,
    _: bool = Depends(get_admin_auth)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create/update model cost: {str(e)}")
